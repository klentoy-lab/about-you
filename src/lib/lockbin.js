...
export class LockbinError extends Error {}

// Wrong-passcode protection shared by every place that checks it (Lockbin view and Settings).
let failures = 0
let frozenUntil = 0

/** Checks a passcode without opening the Lockbin. Five wrong tries freeze checks for 30 seconds. */
export async function verifyPasscode(passcode) {
  const lb = getSettings().lockbin
  if (!lb) throw new LockbinError('No passcode is set')
  if (Date.now() < frozenUntil) {
    throw new LockbinError(`Too many tries — wait ${Math.ceil((frozenUntil - Date.now()) / 1000)} seconds`)
  }
  ...
  if (failures >= 5) {
    failures = 0
    frozenUntil = Date.now() + 30_000
    throw new LockbinError('Too many tries — wait 30 seconds')
  }
  return false
}

export async function setPasscode(passcode) {
  if (passcode.length < MIN_PASSCODE) throw new LockbinError(`At least ${MIN_PASSCODE} characters`)
  const salt = toHex(crypto.getRandomValues(new Uint8Array(16)))
  saveSettings({ lockbin: { salt, hash: await hash(passcode, salt) } })
  open()
}

/** Replaces the passcode after confirming the current one. */
export async function changePasscode(current, next) {
  if (!(await verifyPasscode(current))) throw new LockbinError('Current passcode is wrong')
  await setPasscode(next)
}

/**
 * Removes the passcode after confirming it. Refused while anything is sealed,
 * so removing it can never quietly expose Lockbin entries.
 */
export async function removePasscode(current, sealedCount) {
  if (sealedCount > 0) {
    throw new LockbinError(`Move the ${sealedCount} sealed ${sealedCount === 1 ? 'entry' : 'entries'} out of the Lockbin first`)
  }
  if (!(await verifyPasscode(current))) throw new LockbinError('Passcode is wrong')
  saveSettings({ lockbin: null })
  lock()
}
