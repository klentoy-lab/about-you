...
function InlineVideo({ url, onExpand, label }) {
  ...
  return (
    <>
      ...
      />
      <span className="type-meta pointer-events-none absolute bottom-2 left-2 bg-ink px-1.5 py-0.5 text-[#FFF1D6]">
        <span className="print:hidden">{!playing ? 'Play' : muted ? 'Sound off' : 'Sound on'}</span>
        <span className="hidden print:inline">▶ Video</span>
      </span>
      <button
        type="button"
        onClick={onExpand}
        aria-label={`Open ${label} full screen`}
        className="type-meta absolute right-2 top-2 bg-ink px-1.5 py-0.5 text-[#FFF1D6] hover:text-mango print:hidden"
      >
        Full
      </button>
    </>
