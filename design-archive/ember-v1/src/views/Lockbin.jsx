...
import { hasPasscode, lock, setPasscode, unlock, useLockbin } from '../lib/lockbin.js'
...
function Unlock() {
  ...
  const submit = async (e) => {
    e.preventDefault()
    if (Date.now() < frozenUntil) return setError('Too many tries — wait a moment')
    setBusy(true)
  ...
  return (
    <>
      ...
      {entries.length === 0 ? (
        ...
        <ol className="mt-12 max-w-[64rem]">
          {entries.map((e) => (
            <li key={e.id} className="rule-hairline">
              <EntryFull entry={e} href={`#/today/${e.date}`} showVisibility />
              <p className="type-meta -mt-6 pb-10 pl-6 md:pl-8">
                <button type="button" onClick={() => setInLockbin(e.date, false)} className="text-mango hover:text-vanilla">
